...
den nächsten Job an. **Ein ESP32-C3 bedient zwei Drucker.** Die Weboberfläche gibt es
auf Deutsch und Englisch (Umschalter oben rechts; das Board merkt sich die Wahl).
...
| Tastendruck wurde Minuten später ausgeführt, wenn das WLAN zurückkam | Druck blieb unbegrenzt in der Warteschlange | Druck verfällt nach 30 s; die LED flackert kurz, wenn er nicht zugestellt werden konnte |
...
3. Dein WLAN aus der Liste wählen (nur 2,4 GHz), Passwort eingeben, optional den
   ...
   `http://bambutton.local/`), bei Misserfolg der Grund. Schließt das Handy die
   Setup-Ansicht beim Umschalten, einfach wieder öffnen.
4. Handy oder PC wieder mit dem Heimnetz verbinden und `http://bambutton.local/`
   (oder die angezeigte IP) öffnen. Das Setup-Netz schließt sich von selbst, sobald du
   es verlassen hast (frühestens 90 s nach dem Verbinden, spätestens nach 5 Minuten),
   oder sofort über den Knopf auf der Seite.
...
- `http://bambutton.local/` funktioniert auf iPhone, Mac, Windows 10/11 und den meisten
  ...
  Router unter dem Hostnamen (Standard `bambutton`) nachsehen. Ein geänderter Name
  erscheint im Router nach der nächsten Verbindung.
...
GitHub Actions baut die Firmware bei jedem Push auf `main`, bei Pull Requests und bei
Tags (`v*`); die Binärdateien werden an jeden Tag als Release angehängt und die
Flash-Seite wird von `main` auf GitHub Pages veröffentlicht.
